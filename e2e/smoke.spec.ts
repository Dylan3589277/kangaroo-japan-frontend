...
test.describe("localized storefront smoke coverage", () => {
  test("home page loads key navigation and search", async ({ page }) => {
    ...
    await expect(page.locator('header a[href$="/products"]').first()).toBeVisible();
  ...
  test("products page loads mocked products", async ({ page }) => {
    ...
    await expect(page.locator('form#search-form input[type="text"]')).toBeVisible();
  });
  ...
  test("cart page loads empty authenticated cart state", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /购物车是空的|Your cart is empty/i })).toBeVisible();
    await expect(page.locator('a[href$="/products"]').first()).toBeVisible();
  });

  test("login page exposes required credentials form controls", async ({ page }) => {
    await mockBackend(page);
  ...
  test("support contact page loads tawk.to widget without exposing order data", async ({ page }) => {
    await mockBackend(page);
    ...
    await expect(page.locator('script#tawkto-widget[src*="embed.tawk.to"]')).toHaveCount(1);
    expect(await page.locator('script#tawkto-widget').getAttribute("src")).not.toContain("order");
  });

  test("admin support page can run masked readonly order lookup", async ({ page }) => {
    await signInForE2E(page, "admin");
    await mockBackend(page);

    await page.goto("/zh/admin/support");

    await expect(page.getByRole("heading", { name: "客服工单台账" })).toBeVisible();
    await expect(page.locator('script[src*="embed.tawk.to"]')).toHaveCount(0);
    await page.getByLabel("订单号").fill("DSJ-E2E-0001");
    await page.getByRole("button", { name: "查询" }).click();

    await expect(page.getByText("DSJ-E2E-0001")).toBeVisible();
    await expect(page.getByText("e***@example.com")).toBeVisible();
    await expect(page.getByText("YA***0001")).toBeVisible();
    await expect(page.getByText("仓库发货单：pending / EMS")).toBeVisible();
  });

  test("checkout pre-payment page loads cart, address, and order controls", async ({ page }) => {
    await signInForE2E(page);
    ...
    await expect(page.getByText("Payment will be processed after order confirmation")).toBeVisible();
  });
});
