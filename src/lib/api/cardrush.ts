/**
 * CardRush (カードラッシュ ポケモン) API helpers.
 *
 * 后端 detail 端点：GET /api/v1/integrations/cardrush/detail?id=<id>
 * 后端 list   端点：GET /api/v1/integrations/cardrush/list?keyword=&page=
 * 后端 quote  端点：GET /api/v1/cardrush/quote?goodsNo=<id>&tcg=true|false
 *
 * 返回结构 MarketplaceItem = 后端 integrations 统一 schema（与 rakuma/yahoofrima 共用）。
 *
 * 复用 rakuma.ts 的 MarketplaceItem / Quote 类型与归一逻辑；CardRush 商品 id 为纯数字
...
/** CardRush quote 响应（与 RakumaQuote 同构，便于复用结算组件）。 */
export type CardrushQuote = RakumaQuote;

/**
 * 拉 CardRush 商品详情。
 * 经 /api/backend 代理 → 后端 GET /api/v1/integrations/cardrush/detail?id=...
 * lng 传 "zh" 时后端按需调用 Azure Translator 附带 titleTranslated/descriptionTranslated
 * （未配 Azure key 时后端原样忽略，不影响原有字段）。
 */
export async function getCardrushDetail(id: string, lng?: string) {
  const lngQuery = lng ? `&lng=${encodeURIComponent(lng)}` : "";
  return api.request<MarketplaceItem>(
    `/integrations/cardrush/detail?id=${encodeURIComponent(id)}${lngQuery}`,
  );
}

/** CardRush 搜索列表卡片（与 RakumaCardItem 同形，便于复用列表组件）。 */
export interface CardrushCardItem {
  /** 商品数字 id；用于跳转 /[lang]/cardrush/[id]。 */
  goodsNo: string;
  title: string;
  imageUrl?: string;
...
function normalizeCardrushItem(value: unknown): CardrushCardItem | null {
...
export interface SearchCardrushResult {
  items: CardrushCardItem[];
  totalPages: number;
}

/**
 * 拉 CardRush 在售搜索列表并归一成 CardrushCardItem[]。
 * 经 /api/backend 代理 → 后端 GET /api/v1/integrations/cardrush/list?keyword=&page=
 * （后端实时抓 cardrush-pokemon.jp 公开搜索页，短缓存、限频、熔断；不落库）。
 * 失败/无数据 → { items: [], totalPages: 0 }（调用方友好兜底，不抛错）。
 */
export async function searchCardrush(
  keyword: string,
  page = 1,
): Promise<SearchCardrushResult> {
  const trimmed = (keyword || "").trim();
  if (!trimmed) return { items: [], totalPages: 0 };
  try {
    const res = await api.request(
      `/integrations/cardrush/list?keyword=${encodeURIComponent(trimmed)}&page=${page}`,
    );
    if (!res.success || !res.data) return { items: [], totalPages: 0 };
    const data = asRecord(res.data);
    const items = extractGoodsList(res.data)
      .map(normalizeCardrushItem)
      .filter((item): item is CardrushCardItem => item !== null);
...
/**
 * 拉 CardRush 报价（手续费 + 汇率换算）。
 * 经 /api/backend 代理 → 后端 GET /api/v1/cardrush/quote?goodsNo=...
 * opts.tcg=true 仅 en（TCG）页面传：返回 amountUsd/priceUsd；
 * zh 经典页不传（默认），走旧端动态价（暂未开放亦兜底）。
 */
export async function getCardrushQuote(
  goodsNo: string,
  opts?: { tcg?: boolean },
) {
  const tcgQuery = opts?.tcg ? "&tcg=true" : "";
  return api.request<CardrushQuote>(
    `/cardrush/quote?goodsNo=${encodeURIComponent(goodsNo)}${tcgQuery}`,
  );
}
