 ...
 * 缘由（花哥 2026-07-21）：代购手续费三家都现免（都 0）、免费仓储不硬拉 90（免得日本仓
 * 压力过大），这两项拿去和竞品比是硬凑、没意义——改成只讲自己的资历（10 年代拍老店）和实价。
 * 2026-07-26 花哥更新仓储口径：**30–60 天，按会员等级而定**（原写死「30天」是基础档单值，
 * 与 FAQ/费用页新口径不一致，客户会看到两个数字）。同步 zh 信任页文案一起改。
 */
...
export const HIGHLIGHTS: { label: string; value: string; note?: string }[] = [
  ...
  { label: "免费仓储", value: "30-60天", note: "按会员等级" },
];
...
export const COMPARE_ROWS: CompareRow[] = [
  {
    label: "客服方式",
    ours: "智能+真人客服",
    // 2026-07-26 更正：原写「24小时」与真实口径冲突——真人客服实为
    // 周一至周日 9:00-18:00（**中国时间**，非 JST；客服系统 chat/route.ts 一直是这个口径）。
    // 在一张对比竞品的表里把自己写成 24 小时、实际不是，比写错竞品更危险（虚假宣传，
    // 且对手可直接举证）。改为分开陈述：AI 全天、真人限时段。
    oursSub: "AI全天·真人9-18",
    competitorA: "在线客服",
    competitorB: "人工客服·限时段",
    source: "花哥拍板：客服方式对比；我方真人时段口径「周一至周日 9:00-18:00 中国时间」（=日本时间 10:00-19:00），与 chat/route.ts 客服系统口径一致；⚠️挖★姬「机器人客服」据第三方用户反馈(黑猫/知乎)非官方口径，对外发布前需评估措辞法律风险；乐★番人工09:30-18:30+智能全天据官网，2026-07-21。",
  },
  {
    label: "帮砍价·代留言",
    ours: "免费",
