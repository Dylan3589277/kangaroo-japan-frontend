...
/**
 * /[lang]/yahoofrima — PayPayフリマ 落地页（粘贴商品链接 → 跳详情）。
 *
 * PayPayフリマ 没有列表/搜索 API（详情靠抓单个商品网页），因此本页提供
 * "粘贴商品链接"入口：解析出 itemId → router.push(/[lang]/yahoofrima/[itemId])。
 *
 ...
 * 带/不带 query、末尾斜杠、以及直接粘一串纯 itemId。
 * 解析不出返回 null。
 */
function parseYahoofrimaId(raw: string): string | null {
...
export default function YahoofrimaLandingPage({
  params,
}: {
  ...
  const t = useTranslations("yahoofrima.paste");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const itemId = parseYahoofrimaId(value);
    if (!itemId) {
      setError(true);
      return;
    }
