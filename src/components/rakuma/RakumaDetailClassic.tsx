 ...
 * zh locale 渲染。来源显示「楽天 Rakuma」，价格显人民币估算。
 * 数据接后端 /integrations/rakuma/detail + /rakuma/quote（tcg=false）。
 * 视觉与布局严格对齐 MercariDetailClassic，仅平台标识与数据源不同。
 */
export function RakumaDetailClassic() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await getRakumaDetail(id, "zh");
        if (!active) return;
        if (res.success && res.data) {
          setDetail(res.data as MarketplaceItem);
        }
      } catch (error) {
        console.error("Failed to fetch Rakuma detail:", error);
      } finally {
    ...
    const fetchFee = async () => {
      try {
        const res = await getRakumaQuote(id, { tcg: false });
        if (!active) return;
  ...
  const toggleCollect = async () => {
    try {
      const res = await api.request(`/integrations/rakuma/collect`, {
        method: "POST",
  ...
  const openKefu = () => {
    ...
    openWithProduct({
      ...
      platform: "mercari",
      href: `/${lang}/checkout?type=rakuma&id=${id}`,
    });
  };
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/rakuma" className="hover:text-primary">
              楽天 Rakuma
            </Link>
          </li>
          <li>/</li>
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">楽天 Rakuma</span>
              </div>
              <div className="flex justify-between">
