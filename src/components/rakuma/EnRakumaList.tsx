 ...
 * 复用 Mercari 设计 A 的暗色版式，数据源 GET /integrations/rakuma/list。
 * 顶部保留「粘贴商品链接」入口（→ /[lang]/rakuma/[hash]）。
...
function sortItems(items: RakumaCardItem[], sort: SortKey): RakumaCardItem[] {
...
function EnRakumaCard({
  ...
  return (
    <Link
      href={`/rakuma/${encodeURIComponent(item.goodsNo)}`}
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03] transition-all hover:-translate-y-0.5 hover:border-cyan-400/40 ${
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-white/[0.04]">
        ...
        <span className="absolute left-2 top-2 inline-flex items-center rounded-md bg-cyan-400 px-2 py-0.5 text-[10px] font-semibold text-[#06121b] shadow-sm">
          Rakuma
        </span>
...
export function EnRakumaList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await searchRakuma(submittedKeyword, 1);
        if (!active) return;
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-white/15 bg-white/[0.04] px-6 pr-24 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-400"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-cyan-400 px-6 text-sm font-semibold text-[#06121b] transition-colors hover:bg-cyan-300"
              >
                {t("submit")}
              </button>
            </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pt-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-slate-400">
            <span className="inline-block h-4 w-1 rounded-full bg-cyan-400" />
            <span>
          ...
          <label className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <select
              ...
              className="rounded-full border border-white/12 bg-white/[0.04] px-4 py-1.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-400"
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-cyan-400 px-5 py-2 text-sm font-semibold text-[#06121b] transition-colors hover:bg-cyan-300"
            >
              {t("seePopular")}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {sortedItems.map((item) => (
              <EnRakumaCard key={item.goodsNo} item={item} t={t} />
            ))}
          </div>
        )}
      </section>
