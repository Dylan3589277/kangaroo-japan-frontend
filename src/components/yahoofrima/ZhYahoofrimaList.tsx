 ...
 * 复用 Mercari/Rakuma zh 列表版式/交互，数据源换成 GET /integrations/yahoofrima/list。
...
function ZhYahoofrimaCard({
  ...
  return (
    <Link
      ...
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-zinc-100 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#ff0033]/30 hover:shadow-md ${
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-2.5">
        ...
        <div className="mt-auto flex items-baseline gap-1.5">
          {typeof item.priceJpy === "number" ? (
            <span className="text-sm font-bold text-[#d6002b]">
...
function ZhYahoofrimaCardSkeleton() {
  return (
...
export function ZhYahoofrimaList() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 pb-12">
      <section className="bg-gradient-to-b from-[#fff0f3] to-zinc-50 pb-6 pt-8">
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-zinc-200 bg-white px-6 pr-24 text-zinc-900 placeholder:text-zinc-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ff0033]"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-[#ff0033] px-6 text-sm font-medium text-white transition-colors hover:bg-[#e6002e]"
              >
                {t("submit")}
              </button>
            </div>
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs transition-colors ${
                  submittedKeyword === hot.keyword && !keyword
                    ? "border-[#ff0033] bg-[#fff0f3] text-[#d6002b]"
                    : "border-zinc-200 bg-white text-zinc-600 hover:border-[#ff0033]/60 hover:text-[#d6002b]"
                }`}
              >
                {hot.label}
              </button>
      ...
      <section className="mx-auto w-full max-w-7xl px-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-zinc-500">
            <span className="inline-block h-4 w-1 rounded-full bg-gradient-to-b from-[#ff0033] to-[#ff6680]" />
            <span>
          ...
          <label className="flex items-center gap-2 text-sm text-zinc-600">
            ...
            <select
              ...
              className="rounded-full border border-zinc-200 bg-white px-4 py-1.5 text-sm text-zinc-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ff0033]"
            >
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {Array.from({ length: 10 }).map((_, i) => (
              <ZhYahoofrimaCardSkeleton key={i} />
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-white py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-[#ff0033] px-5 py-2 text-sm font-medium text-white transition-colors hover:bg-[#e6002e]"
            >
              {t("seePopular")}
            </button>
          </div>
