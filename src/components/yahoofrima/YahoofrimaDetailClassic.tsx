...
/**
 * 经典版（浅色通用样式）PayPayフリマ 商品详情页。
 * zh locale 渲染。来源显示「PayPay フリマ」，价格显人民币估算。
 * 数据接后端 /integrations/yahoofrima/detail + /yahoofrima/quote（tcg=false）。
 * 视觉与布局严格对齐 MercariDetailClassic，仅平台标识与数据源不同。
 */
export function YahoofrimaDetailClassic() {
  ...
  // 动态手续费（来自后端 quote），未登录/报价失败时为 null。
  const [feeJpy, setFeeJpy] = useState<number | null>(null);
  // 人民币应付（后端 quote 的 amountRmb），未登录/无报价时为 null。
  ...
  useEffect(() => {
    ...
    const fetchFee = async () => {
      try {
        const res = await getYahoofrimaQuote(id, { tcg: false });
        if (!active) return;
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/yahoofrima" className="hover:text-primary">
              PayPay フリマ
            </Link>
          </li>
          <li>/</li>
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">PayPay フリマ</span>
              </div>
              <div className="flex justify-between">
