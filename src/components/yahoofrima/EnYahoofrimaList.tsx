...
/**
 * en（设计 A，深色，面向海外）PayPayフリマ 搜索 + 列表页。
 * 复用 Mercari/Rakuma 设计 A 的暗色版式，数据源 GET /integrations/yahoofrima/list。
 * 顶部保留「粘贴商品链接」入口（→ /[lang]/yahoofrima/[itemId]）。
 * 配色：Yahoo 红角标（与 Mercari/Rakuma 区分平台）。
...
function EnYahoofrimaCard({
  ...
  return (
    <Link
      href={`/yahoofrima/${encodeURIComponent(item.goodsNo)}`}
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03] transition-all hover:-translate-y-0.5 hover:border-[#ff0033]/40 ${
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-2.5">
        ...
        <div className="mt-auto flex items-baseline gap-1.5">
          {typeof item.priceJpy === "number" ? (
            <span className="text-sm font-bold text-[#ff5470]">
...
function EnYahoofrimaCardSkeleton() {
  return (
...
export function EnYahoofrimaList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await searchYahoofrima(submittedKeyword, 1);
        if (!active) return;
        ...
        <div className="mx-auto w-full max-w-7xl px-4">
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-white/15 bg-white/[0.04] px-6 pr-24 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#ff0033]"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-[#ff0033] px-6 text-sm font-semibold text-white transition-colors hover:bg-[#e6002e]"
              >
                {t("submit")}
              </button>
            </div>
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs transition-colors ${
                  submittedKeyword === hot.keyword && !keyword
                    ? "border-[#ff0033]/50 bg-[#ff0033]/10 text-[#ff5470]"
                    : "border-white/10 bg-white/[0.03] text-slate-400 hover:border-[#ff0033]/30 hover:text-[#ff5470]"
                }`}
              >
                {hot.label}
              </button>
          ...
          >
            <input
              ...
              className="h-10 flex-1 rounded-full border border-white/12 bg-white/[0.03] px-5 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#ff0033]/60"
            />
            <button
              type="submit"
              className="h-10 shrink-0 rounded-full border border-[#ff0033]/40 bg-[#ff0033]/10 px-5 text-sm font-medium text-[#ff5470] transition-colors hover:bg-[#ff0033]/20"
            >
              {t("pasteSubmit")}
            </button>
          </form>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pt-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-slate-400">
            <span className="inline-block h-4 w-1 rounded-full bg-[#ff0033]" />
            <span>
          ...
          <label className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <select
              ...
              className="rounded-full border border-white/12 bg-white/[0.04] px-4 py-1.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-[#ff0033]"
            >
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {Array.from({ length: 10 }).map((_, i) => (
              <EnYahoofrimaCardSkeleton key={i} />
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-[#ff0033] px-5 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#e6002e]"
            >
              {t("seePopular")}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {sortedItems.map((item) => (
              <EnYahoofrimaCard key={item.goodsNo} item={item} t={t} />
            ))}
          </div>
        )}
      </section>
