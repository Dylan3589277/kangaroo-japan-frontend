...
/**
 * 经典版（浅色 + 红色卡牌点缀）CardRush カードラッシュ 商品详情页。
 * zh locale 渲染。来源显示「CardRush」，价格显人民币估算。
 * 数据接后端 /integrations/cardrush/detail + /cardrush/quote（tcg=false）。
 * 视觉与布局对齐 rakuma 经典版，主题点缀由橙改红、平台标识为 CardRush。
 * i18n 复用 rakuma 命名空间（通用文案）。
 */
export function CardrushDetailClassic() {
  ...
  useEffect(() => {
    ...
    const fetchFee = async () => {
      try {
        const res = await getCardrushQuote(id, { tcg: false });
        if (!active) return;
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/cardrush" className="hover:text-primary">
              CardRush
            </Link>
          </li>
          <li>/</li>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          <div className="group relative aspect-square bg-muted rounded-lg overflow-hidden mb-4">
            ...
            <span className="absolute top-4 left-4 inline-flex items-center rounded-md bg-red-600 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-white shadow-sm">
              CardRush
            </span>
            {soldOut && (
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">CardRush</span>
              </div>
              <div className="flex justify-between">
