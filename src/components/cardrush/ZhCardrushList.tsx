...
/**
 * zh（经典/非 en）CardRush カードラッシュ 搜索 + 列表页（中国导购风 + 红色卡牌点缀）。
 * 复用 rakuma ZhRakumaList 版式/交互，数据源换成 GET /integrations/cardrush/list，
 * 主题点缀改红色卡牌风、平台角标显「CardRush」。
...
function ZhCardrushCard({
  ...
  return (
    <Link
      ...
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-zinc-100 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-red-200 hover:shadow-md ${
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-zinc-100">
        ...
        <span className="absolute left-2 top-2 inline-flex items-center rounded-md bg-red-600 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white shadow-sm">
          CardRush
        </span>
...
function ZhCardrushCardSkeleton() {
  return (
...
export function ZhCardrushList() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 pb-12">
      <section className="bg-gradient-to-b from-red-50 to-zinc-50 pb-6 pt-8">
        <div className="mx-auto w-full max-w-7xl px-4">
          <h1 className="mb-1 flex items-center gap-2 text-2xl font-bold text-zinc-900">
            <span className="inline-flex h-6 items-center rounded-md bg-red-600 px-2 text-xs font-bold uppercase tracking-wider text-white">
              CardRush
            </span>
            {t("heading")}
          </h1>
          <p className="mb-4 text-xs text-zinc-400">
            宝可梦卡专营 · cardrush-pokemon.jp
          </p>
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-zinc-200 bg-white px-6 pr-24 text-zinc-900 placeholder:text-zinc-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-red-600 px-6 text-sm font-medium text-white transition-colors hover:bg-red-700"
              >
                {t("submit")}
              </button>
            </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm text-zinc-500">
            <span className="inline-block h-4 w-1 rounded-full bg-gradient-to-b from-red-600 to-red-400" />
            <span>
          ...
          <label className="flex items-center gap-2 text-sm text-zinc-600">
            ...
            <select
              ...
              className="rounded-full border border-zinc-200 bg-white px-4 py-1.5 text-sm text-zinc-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500"
            >
