 ...
 * zh locale 渲染。来源显示「トレカキャンプ」，价格显人民币估算。
 * 数据接后端 /integrations/torecacamp/detail + /torecacamp/quote（tcg=false）。
 * 视觉与布局严格对齐 MercariDetailClassic，仅平台标识与数据源不同。
 */
export function TorecacampDetailClassic() {
  ...
  useEffect(() => {
    ...
    const fetchFee = async () => {
      try {
        const res = await getTorecacampQuote(id, { tcg: false });
        if (!active) return;
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/torecacamp" className="hover:text-primary">
              トレカキャンプ
            </Link>
          </li>
          <li>/</li>
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">トレカキャンプ</span>
              </div>
              <div className="flex justify-between">
