 ...
 * 复用 Mercari 设计 A 的暗色版式，数据源 GET /integrations/torecacamp/list。
 * 顶部保留「粘贴商品链接」入口（→ /[lang]/torecacamp/[hash]）。
 * en 站显 USD（后端 price_usd，汇率不可用时只显 JPY，绝不显错币）。金额一律 JPY 整数。
 */

// torecacamp is a Pokémon-card-only Shopify store; the keyword filter runs over the bounded
// /products.json scan, so defaults must be terms that actually appear in the catalogue.
// (Verified live 2026-06-24: PSA, ADV, ex, SAR, VMAX all return real cards in-scan.)
const DEFAULT_KEYWORD = "PSA";

type SortKey = "newest" | "priceAsc" | "priceDesc";

const HOT_KEYWORDS: { label: string; keyword: string }[] = [
  { label: "PSA Graded", keyword: "PSA" },
...
function EnTorecacampCard({
  item,
  t,
}: {
  item: TorecacampCardItem;
  ...
  return (
    <Link
      href={`/torecacamp/${encodeURIComponent(item.goodsNo)}`}
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03] transition-all hover:-translate-y-0.5 hover:border-emerald-400/40 ${
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-white/[0.04]">
        ...
        <span className="absolute left-2 top-2 inline-flex items-center rounded-md bg-emerald-400 px-2 py-0.5 text-[10px] font-semibold text-[#06121b] shadow-sm">
          Torecacamp
        </span>
...
function EnTorecacampCardSkeleton() {
  return (
...
export function EnTorecacampList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await searchTorecacamp(submittedKeyword, 1);
        if (!active) return;
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-white/15 bg-white/[0.04] px-6 pr-24 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-400"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-emerald-400 px-6 text-sm font-semibold text-[#06121b] transition-colors hover:bg-emerald-300"
              >
                {t("submit")}
              </button>
            </div>
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs transition-colors ${
                  submittedKeyword === hot.keyword && !keyword
                    ...
                    : "border-white/10 bg-white/[0.03] text-slate-400 hover:border-emerald-400/30 hover:text-emerald-300"
                }`}
              >
                {hot.label}
              </button>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <select
              ...
              className="rounded-full border border-white/12 bg-white/[0.04] px-4 py-1.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400"
            >
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {Array.from({ length: 10 }).map((_, i) => (
              <EnTorecacampCardSkeleton key={i} />
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-emerald-400 px-5 py-2 text-sm font-semibold text-[#06121b] transition-colors hover:bg-emerald-300"
            >
              {t("seePopular")}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {sortedItems.map((item) => (
              <EnTorecacampCard key={item.goodsNo} item={item} t={t} />
            ))}
          </div>
        )}
      </section>
