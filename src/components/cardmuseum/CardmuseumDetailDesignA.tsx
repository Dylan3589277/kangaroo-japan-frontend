...
import { getCardmuseumDetail, getCardmuseumQuote } from "@/lib/api/cardmuseum";
import type { MarketplaceItem } from "@/lib/api/cardmuseum";

/**
 * 设计 A（红黑卡牌风）英文 Card Museum カードラッシュ 商品详情页。
 * en locale 渲染。显美元，TCG quote（tcg=true）。
 * 视觉对齐 rakuma 设计 A 暗色版式，主题色由电光青改为红黑卡牌风（teal-500 强调 + 黑底）。
 * 来源标签显「Card Museum」。i18n 复用 rakuma 命名空间（通用文案）。
 */
export function CardmuseumDetailDesignA() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await getCardmuseumDetail(id);
        if (!active) return;
        if (res.success && res.data) {
          setDetail(res.data as MarketplaceItem);
        }
      } catch (error) {
        console.error("Failed to fetch Card Museum detail:", error);
      } finally {
    ...
    const fetchFee = async () => {
      try {
        const res = await getCardmuseumQuote(id, { tcg: true });
        if (!active) return;
  ...
  const toggleCollect = async () => {
    try {
      ...
      const res = await api.request(`/integrations/cardmuseum/collect`, {
        method: "POST",
  ...
  // 网页代拍下单：加购与立即购买都进通用代拍结算（type=cardmuseum）。
  // Card Museum 商品为实时抓取、非 products 行，没有 products-FK 购物车可承载；
  // 故加购直接进结算（建单→付款→人工履约）。
  const goToCheckout = () => {
    router.push(`/${lang}/checkout?type=cardmuseum&id=${id}`);
  };
    ...
    return (
      <main className={`${spaceGrotesk.variable} flex min-h-[60vh] flex-col items-center justify-center bg-[#0a0e16] px-4 text-center text-slate-200`}>
        ...
        <button
          type="button"
          onClick={() => router.back()}
          className="mt-6 inline-flex h-11 items-center rounded-xl bg-teal-600 px-5 text-sm font-semibold text-white transition-colors hover:bg-teal-500"
        >
          {t("emptyBack")}
        </button>
      </main>
  ...
  return (
    <main className={`${spaceGrotesk.variable} min-h-screen bg-[#0a0e16] text-slate-200 antialiased`}>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(55% 60% at 50% -10%, rgba(20,184,166,0.16), transparent 60%)",
        }}
      />
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-36 pt-6 md:pb-12">
        ...
        <nav className="mb-6 text-xs">
          <ol className="flex items-center gap-2 text-slate-500">
            ...
            <li aria-hidden>/</li>
            <li>
              <Link href="/cardmuseum" className="transition-colors hover:text-emerald-300">
                Card Museum
              </Link>
            </li>
            <li aria-hidden>/</li>
        ...
        <div className="grid gap-8 md:grid-cols-2 md:items-start">
          ...
          <div className="md:sticky md:top-20">
            <div className="group relative aspect-square overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0e131d]">
              {images.length > 0 ? (
                ...
                >
                  ...
                  <span className="pointer-events-none absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded-md bg-black/55 px-2 py-1 text-[10px] font-medium text-slate-200 backdrop-blur">
                    <svg className="size-3.5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
              ...
              <span className="pointer-events-none absolute left-3 top-3 inline-flex items-center rounded-md bg-teal-600 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white ring-1 ring-emerald-400/40">
                Card Museum
              </span>
              {soldOut && (
          ...
          <section className="flex flex-col">
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03]">
              ...
              <dl className="divide-y divide-white/[0.06] text-sm">
                ...
                <div className="flex items-center justify-between bg-teal-500/[0.06] px-5 py-4">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 text-center">
              <Link
                ...
                className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3 transition-colors hover:border-teal-500/40"
              >
                <CameraIcon className="mx-auto size-5 text-emerald-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.inspection")}
                </p>
              </Link>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShieldIcon className="mx-auto size-5 text-emerald-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.protection")}
                </p>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShipIcon className="mx-auto size-5 text-emerald-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <button
                ...
                className={`inline-flex h-10 items-center gap-2 rounded-xl border px-4 text-sm transition-colors ${
                  isCollected
                    ? "border-emerald-400/40 bg-teal-500/10 text-emerald-300"
              ...
              <Link
                href="/photo-inspection"
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-slate-200 transition-colors hover:border-teal-500/40 hover:text-emerald-200"
              >
              ...
              <button
                ...
                onClick={() =>
                  openWithProduct({
                    ...
                    platform: "mercari",
                    href: `/${lang}/checkout?type=cardmuseum&id=${id}`,
                  })
                }
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-slate-200 transition-colors hover:border-teal-500/40 hover:text-emerald-200"
              >
            ...
            <div className="mt-5 hidden gap-3 md:flex">
              <button
                ...
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl border border-white/15 bg-white/[0.04] text-sm font-medium text-slate-100 transition-colors hover:border-teal-500/40 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {t("addToCart")}
              </button>
              <button
                ...
                onClick={goToCheckout}
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl bg-teal-600 text-base font-semibold text-white transition-colors hover:bg-teal-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
              >
