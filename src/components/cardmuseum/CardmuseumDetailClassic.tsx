...
/**
 * 经典版（浅色 + 红色卡牌点缀）Card Museum カードラッシュ 商品详情页。
 * zh locale 渲染。来源显示「Card Museum」，价格显人民币估算。
 * 数据接后端 /integrations/cardmuseum/detail + /cardmuseum/quote（tcg=false）。
 * 视觉与布局对齐 rakuma 经典版，主题点缀由橙改红、平台标识为 Card Museum。
 * i18n 复用 rakuma 命名空间（通用文案）。
 */
export function CardmuseumDetailClassic() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await getCardmuseumDetail(id, "zh");
        if (!active) return;
    ...
    const fetchFee = async () => {
      try {
        const res = await getCardmuseumQuote(id, { tcg: false });
        if (!active) return;
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/cardmuseum" className="hover:text-primary">
              Card Museum
            </Link>
          </li>
          <li>/</li>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          <div className="group relative aspect-square bg-muted rounded-lg overflow-hidden mb-4">
            ...
            <span className="absolute top-4 left-4 inline-flex items-center rounded-md bg-teal-600 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-white shadow-sm">
              Card Museum
            </span>
            {soldOut && (
              ...
              {images.map((img, idx) => (
                <button
                  ...
                  className={`relative w-20 h-20 flex-shrink-0 rounded-md overflow-hidden border-2 ${
                    selectedImage === idx ? "border-teal-500" : "border-transparent"
                  }`}
                >
                  <Image
                    src={img}
        ...
        <div>
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">Card Museum</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <Button
              ...
              className={`gap-2 ${isCollected ? "text-teal-500 border-emerald-200" : ""}`}
            >
