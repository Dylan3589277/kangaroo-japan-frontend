...
import { ArrowRight, Plane, Search, Warehouse } from "lucide-react";
            ...
            {STEPS.map((step, i) => (
              <Fragment key={step.title}>
                <div className="flex flex-1 items-start gap-4 sm:flex-col sm:items-center sm:gap-2 sm:text-center">
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-rose-50 text-rose-600">
                    {i === 2 ? (
                      <MascotScene name="ship" alt={step.title} size={44} />
                    ) : (
