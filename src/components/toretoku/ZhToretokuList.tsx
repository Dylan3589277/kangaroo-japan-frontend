 ...
 * 复用 cardrush ZhCardrushList 版式/交互，数据源换成 GET /integrations/toretoku/list，
 * 主题点缀改橙红激安风、平台角标显「トレトク」。トレトク特有：卡片显 型番 + 状態ランク。
  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-zinc-100">
        ...
        {item.conditionRank && (
          <span className="absolute left-2 bottom-2 inline-flex items-center rounded bg-zinc-900/70 px-1.5 py-0.5 text-[10px] font-bold text-orange-200">
            {item.conditionRank}
          </span>
        )}
...
function ZhToretokuCardSkeleton() {
  return (
...
export function ZhToretokuList() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 pb-12">
      <section className="bg-gradient-to-b from-orange-50 to-zinc-50 pb-6 pt-8">
        <div className="mx-auto w-full max-w-7xl px-4">
          <h1 className="mb-1 flex items-center gap-2 text-2xl font-bold text-zinc-900">
            <span className="inline-flex h-6 items-center rounded-md bg-gradient-to-r from-orange-500 to-red-600 px-2 text-xs font-bold tracking-wider text-white">
              トレトク
            </span>
            {t("heading")}
          </h1>
          <p className="mb-4 text-xs text-zinc-400">
            TCG 单卡综合店 · toretoku.jp
          </p>
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-zinc-200 bg-white px-6 pr-24 text-zinc-900 placeholder:text-zinc-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-gradient-to-r from-orange-500 to-red-600 px-6 text-sm font-medium text-white transition-opacity hover:opacity-90"
              >
                {t("submit")}
              </button>
            </div>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-zinc-600">
            ...
            <select
              ...
              className="rounded-full border border-zinc-200 bg-white px-4 py-1.5 text-sm text-zinc-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-white py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-gradient-to-r from-orange-500 to-red-600 px-5 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
            >
              {t("seePopular")}
            </button>
          </div>
