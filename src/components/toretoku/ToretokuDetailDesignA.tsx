...
export function ToretokuDetailDesignA() {
  ...
  useEffect(() => {
    ...
    const fetchFee = async () => {
      try {
        const res = await getToretokuQuote(id, { tcg: true });
        if (!active) return;
  ...
  // トレトク 商品为实时抓取、非 products 行，没有 products-FK 购物车可承载；
  // 故加购直接进结算（建单→付款→人工履约）。
  const goToCheckout = () => {
    ...
    return (
      <main className={`${spaceGrotesk.variable} flex min-h-[60vh] flex-col items-center justify-center bg-[#0a0e16] px-4 text-center text-slate-200`}>
        ...
        <button
          type="button"
          onClick={() => router.back()}
          className="mt-6 inline-flex h-11 items-center rounded-xl bg-gradient-to-r from-orange-500 to-red-600 px-5 text-sm font-semibold text-white transition-opacity hover:opacity-90"
        >
          {t("emptyBack")}
        </button>
      </main>
  ...
  return (
    <main className={`${spaceGrotesk.variable} min-h-screen bg-[#0a0e16] text-slate-200 antialiased`}>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(55% 60% at 50% -10%, rgba(249,115,22,0.16), transparent 60%)",
        }}
      />
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-36 pt-6 md:pb-12">
        ...
        <nav className="mb-6 text-xs">
          <ol className="flex items-center gap-2 text-slate-500">
            ...
            <li aria-hidden>/</li>
            <li>
              <Link href="/toretoku" className="transition-colors hover:text-orange-300">
                トレトク
              </Link>
            </li>
            <li aria-hidden>/</li>
        ...
        <div className="grid gap-8 md:grid-cols-2 md:items-start">
          ...
          <div className="md:sticky md:top-20">
            <div className="group relative aspect-square overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0e131d]">
              {images.length > 0 ? (
                ...
                >
                  ...
                  <span className="pointer-events-none absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded-md bg-black/55 px-2 py-1 text-[10px] font-medium text-slate-200 backdrop-blur">
                    <svg className="size-3.5 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
              ...
              <span className="pointer-events-none absolute left-3 top-3 inline-flex items-center rounded-md bg-gradient-to-r from-orange-500 to-red-600 px-2 py-0.5 text-[10px] font-bold tracking-wide text-white ring-1 ring-orange-400/40">
                トレトク
              </span>
              {soldOut && (
          ...
          <section className="flex flex-col">
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03]">
              ...
              <dl className="divide-y divide-white/[0.06] text-sm">
                ...
                <div className="flex items-center justify-between bg-orange-500/[0.06] px-5 py-4">
                  <dt className="font-semibold text-white">{t("designA.estimatedTotal")}</dt>
                  <dd className="text-right">
                    <div>
                      <span className="text-xl font-bold tabular-nums text-orange-400">
                        {amountJpy !== null
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 text-center">
              <Link
                ...
                className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3 transition-colors hover:border-orange-500/40"
              >
                <CameraIcon className="mx-auto size-5 text-orange-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.inspection")}
                </p>
              </Link>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShieldIcon className="mx-auto size-5 text-orange-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.protection")}
                </p>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShipIcon className="mx-auto size-5 text-orange-400" />
                <p className="mt-1.5 text-[11px] text-slate-400">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <Link
                href="/photo-inspection"
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-slate-200 transition-colors hover:border-orange-500/40 hover:text-orange-200"
              >
            ...
            <div className="mt-5 hidden gap-3 md:flex">
              <button
                ...
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl border border-white/15 bg-white/[0.04] text-sm font-medium text-slate-100 transition-colors hover:border-orange-500/40 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {t("addToCart")}
              </button>
              <button
                ...
                onClick={goToCheckout}
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl bg-gradient-to-r from-orange-500 to-red-600 text-base font-semibold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-400"
              >
