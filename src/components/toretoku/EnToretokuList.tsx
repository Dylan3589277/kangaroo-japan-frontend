...
/**
 * en（设计 A，橙红卡牌风，面向海外 tgc）トレトク 搜索 + 列表页。
 * 复用 cardrush 设计 A 暗色版式，主题色由红改「橙红激安通販风」(orange-500→red-600 强调)，
 * 平台角标显「トレトク」，让买家一眼知道在看 トレトク。数据源 GET /integrations/toretoku/list。
 * トレトク特有：卡片显 型番(modelNumber) + 状態ランク(conditionRank) 角标。
...
function sortItems(items: ToretokuCardItem[], sort: SortKey): ToretokuCardItem[] {
...
function EnToretokuCard({
  item,
  t,
}: {
  item: ToretokuCardItem;
  ...
  return (
    <Link
      href={`/toretoku/${encodeURIComponent(item.goodsNo)}`}
      className={`group flex h-full w-full flex-col overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.03] transition-all hover:-translate-y-0.5 hover:border-orange-500/50 ${
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-white/[0.04]">
        ...
        <span className="absolute left-2 top-2 inline-flex items-center rounded-md bg-gradient-to-r from-orange-500 to-red-600 px-2 py-0.5 text-[10px] font-bold tracking-wide text-white shadow-sm ring-1 ring-orange-400/40">
          トレトク
        </span>

        {item.conditionRank && (
          <span className="absolute left-2 bottom-2 inline-flex items-center rounded bg-black/65 px-1.5 py-0.5 text-[10px] font-bold text-orange-300">
            {item.conditionRank}
          </span>
        )}
...
function EnToretokuCardSkeleton() {
  return (
...
export function EnToretokuList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await searchToretoku(submittedKeyword, 1);
        if (!active) return;
  ...
  return (
    ...
    >
      <section className="border-b border-white/[0.06] bg-gradient-to-b from-orange-950/30 to-transparent pb-6 pt-8">
        <div className="mx-auto w-full max-w-7xl px-4">
          <h1 className="mb-1 flex items-center gap-2 font-[family-name:var(--font-display)] text-2xl font-bold tracking-tight text-white">
            <span className="inline-flex h-6 items-center rounded-md bg-gradient-to-r from-orange-500 to-red-600 px-2 text-xs font-bold tracking-wider text-white ring-1 ring-orange-400/40">
              トレトク
            </span>
            {t("heading")}
          </h1>
          <p className="mb-4 text-xs text-slate-500">
            TCG singles · toretoku.jp
          </p>
          ...
          <form onSubmit={handleSearch} className="mx-auto max-w-2xl">
            <div className="relative">
              <input
                ...
                className="h-12 w-full rounded-full border border-white/15 bg-white/[0.04] px-6 pr-24 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 h-9 -translate-y-1/2 rounded-full bg-gradient-to-r from-orange-500 to-red-600 px-6 text-sm font-semibold text-white transition-opacity hover:opacity-90"
              >
                {t("submit")}
              </button>
            </div>
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs transition-colors ${
                  submittedKeyword === hot.keyword && !keyword
                    ...
                    : "border-white/10 bg-white/[0.03] text-slate-400 hover:border-orange-500/30 hover:text-orange-300"
                }`}
              >
                {hot.label}
              </button>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <select
              ...
              className="rounded-full border border-white/12 bg-white/[0.04] px-4 py-1.5 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-white/[0.02] py-16 text-center">
            ...
            <button
              ...
              className="mt-4 rounded-full bg-gradient-to-r from-orange-500 to-red-600 px-5 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90"
            >
              {t("seePopular")}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {sortedItems.map((item) => (
              <EnToretokuCard key={item.goodsNo} item={item} t={t} />
            ))}
          </div>
        )}
      </section>
