...
import { getSurugayaDetail, getSurugayaQuote } from "@/lib/api/surugaya";
import type { MarketplaceItem } from "@/lib/api/surugaya";

/**
 * 设计 A（深色高级感）英文 駿河屋 Surugaya 商品详情页。
 * en locale 渲染。显美元，TCG quote（tcg=true）。
 * 視覺对齐 Rakuma 设计 A，但用駿河屋蓝橙主题：海军蓝底 + 天蓝 sky-400 + 琥珀 amber-400。
 * 駿河屋核心：价格区同时展示 新品/中古 双价（无该状态价则不显）。来源标签显「駿河屋」。
 */
export function SurugayaDetailDesignA() {
  const params = useParams();
  const router = useRouter();
  const lang = (params.lang as string) || "en";
  const id = params.id as string;
  const t = useTranslations("surugaya");
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setLoading(true);
      try {
        const res = await getSurugayaDetail(id);
        if (!active) return;
        if (res.success && res.data) {
          setDetail(res.data as MarketplaceItem);
        }
      } catch (error) {
        console.error("Failed to fetch Surugaya detail:", error);
      } finally {
    ...
    const fetchFee = async () => {
      try {
        const res = await getSurugayaQuote(id, { tcg: true });
        if (!active) return;
  ...
  const toggleCollect = async () => {
    try {
      ...
      const res = await api.request(`/integrations/surugaya/collect`, {
        method: "POST",
  ...
  // 网页代拍下单：加购与立即购买都进通用代拍结算（type=surugaya）。
  // surugaya 商品为实时抓取、非 products 行，没有 products-FK 购物车可承载；
  // 故加购直接进结算（建单→付款→人工履约），不调不存在的 /integrations/surugaya/cart。
  const goToCheckout = () => {
    router.push(`/${lang}/checkout?type=surugaya&id=${id}`);
  };

  if (loading) {
    return (
      <main className={`${spaceGrotesk.variable} min-h-screen bg-[#0a0f1a] text-slate-200 antialiased`}>
  ...
  if (!detail) {
    return (
      <main className={`${spaceGrotesk.variable} flex min-h-[60vh] flex-col items-center justify-center bg-[#0a0f1a] px-4 text-center text-slate-200`}>
        ...
        <button
          type="button"
          onClick={() => router.back()}
          className="mt-6 inline-flex h-11 items-center rounded-xl bg-sky-500 px-5 text-sm font-semibold text-white transition-colors hover:bg-sky-400"
        >
          {t("emptyBack")}
        </button>
      </main>
  ...
  const hasNew = typeof detail.price_new_jpy === "number";
  const hasUsed = typeof detail.price_used_jpy === "number";
  const hasDual = hasNew || hasUsed;
  ...
  return (
    <main className={`${spaceGrotesk.variable} min-h-screen bg-[#0a0f1a] text-slate-200 antialiased`}>
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-36 pt-6 md:pb-12">
        ...
        <nav className="mb-6 text-xs">
          <ol className="flex items-center gap-2 text-slate-500">
            ...
            <li aria-hidden>/</li>
            <li>
              <Link href="/surugaya" className="transition-colors hover:text-sky-300">
                Surugaya
              </Link>
            </li>
            <li aria-hidden>/</li>
        ...
        <div className="grid gap-8 md:grid-cols-2 md:items-start">
          ...
          <div className="md:sticky md:top-20">
            <div className="group relative aspect-square overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0e1424]">
              {images.length > 0 ? (
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="pointer-events-none absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded-md bg-black/55 px-2 py-1 text-[10px] font-medium text-slate-200 backdrop-blur">
                    <svg className="size-3.5 text-sky-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
              ...
              <span className="pointer-events-none absolute left-3 top-3 inline-flex items-center rounded-md bg-black/55 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-200 backdrop-blur">
                駿河屋
              </span>
              {soldOut && (
          ...
          <section className="flex flex-col">
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.03]">
              <div className="border-b border-white/[0.06] p-5">
                ...
                {hasDual ? (
                  // 駿河屋核心：新品/中古 双价并列。买价(price_jpy) = 有新品取新品否则中古。
                  <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2">
                    {hasNew && (
                      <div>
                        <span className="inline-flex items-center rounded bg-sky-400/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-300">
                          {t("priceNew")}
                        </span>
                        <div className="mt-1 text-2xl font-bold tabular-nums text-sky-300">
                          JPY {numberFormatter.format(Number(detail.price_new_jpy))}
                        </div>
                      </div>
                    )}
                    {hasUsed && (
                      <div>
                        <span className="inline-flex items-center rounded bg-amber-400/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-300">
                          {t("priceUsed")}
                        </span>
                        <div className="mt-1 text-2xl font-bold tabular-nums text-amber-300">
                          JPY {numberFormatter.format(Number(detail.price_used_jpy))}
                          {typeof detail.price_used_max_jpy === "number" && (
                            <span className="text-base font-normal text-amber-400/70">
                              {" ~ "}
                              {numberFormatter.format(detail.price_used_max_jpy)}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <dl className="divide-y divide-white/[0.06] text-sm">
                ...
                <div className="flex items-center justify-between bg-sky-400/[0.04] px-5 py-4">
            ...
            <div className="mt-5 grid grid-cols-3 gap-2 text-center">
              <Link
                ...
                className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3 transition-colors hover:border-sky-400/40"
              >
                <CameraIcon className="mx-auto size-5 text-sky-300" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.inspection")}
                </p>
              </Link>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShieldIcon className="mx-auto size-5 text-sky-300" />
                <p className="mt-1.5 text-[11px] text-slate-400">
                  {t("designA.trust.protection")}
                </p>
              </div>
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-2 py-3">
                <ShipIcon className="mx-auto size-5 text-sky-300" />
                <p className="mt-1.5 text-[11px] text-slate-400">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <Link
                href="/photo-inspection"
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-slate-200 transition-colors hover:border-sky-400/40 hover:text-sky-200"
              >
              ...
              <button
                ...
                onClick={() =>
                  openWithProduct({
                    ...
                    platform: "mercari",
                    href: `/${lang}/checkout?type=surugaya&id=${id}`,
                  })
                }
                className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-sm text-slate-200 transition-colors hover:border-sky-400/40 hover:text-sky-200"
              >
            ...
            <div className="mt-5 hidden gap-3 md:flex">
              <button
                ...
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl border border-white/15 bg-white/[0.04] text-sm font-medium text-slate-100 transition-colors hover:border-sky-400/40 disabled:cursor-not-allowed disabled:opacity-40"
              >
                {t("addToCart")}
              </button>
              <button
                ...
                onClick={goToCheckout}
                disabled={soldOut}
                className="inline-flex h-12 flex-1 items-center justify-center rounded-xl bg-sky-500 text-base font-semibold text-white transition-colors hover:bg-sky-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
              >
      ...
      {/* 底部吸底（移动端） */}
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[0.08] bg-[#0a0f1a]/90 backdrop-blur-md md:hidden">
