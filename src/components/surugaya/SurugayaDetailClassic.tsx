...
/**
 * 经典版（浅色通用样式）駿河屋 Surugaya 商品详情页。
 * zh locale 渲染。来源显示「骏河屋 Surugaya」，价格显人民币估算。
 * 数据接后端 /integrations/surugaya/detail + /proxy-buy/quote（platform=surugaya, tcg=false）。
 * 视觉对齐 Rakuma 经典版，用駿河屋蓝橙主题（sky 主色 + amber 中古价）。
 * 駿河屋核心：价格区同时展示 新品/中古 双价（无该状态价则不显）。
 */
export function SurugayaDetailClassic() {
  ...
  useEffect(() => {
    ...
    const fetchFee = async () => {
      try {
        const res = await getSurugayaQuote(id, { tcg: false });
        if (!active) return;
    ...
    <div className="container mx-auto py-6 px-4 pb-32 md:pb-8">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-muted-foreground">
          ...
          <li>/</li>
          <li>
            <Link href="/surugaya" className="hover:text-primary">
              骏河屋 Surugaya
            </Link>
          </li>
          <li>/</li>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          <div className="group relative aspect-square bg-muted rounded-lg overflow-hidden mb-4">
            ...
            <span className="pointer-events-none absolute top-4 right-4 inline-flex items-center rounded-md bg-sky-600 px-2 py-0.5 text-[10px] font-semibold text-white shadow-sm">
              骏河屋
            </span>
            {soldOut && (
        ...
        <div>
          ...
          <div className="bg-sky-50 rounded-lg p-6 mb-6">
            {hasDual ? (
              ...
              <div className="flex flex-wrap gap-x-8 gap-y-3 mb-2">
                {hasNew && (
                  <div>
                    <span className="inline-flex items-center rounded bg-sky-100 px-1.5 py-0.5 text-[11px] font-semibold text-sky-700">
                      {t("priceNew")}
                    </span>
                    <div className="mt-1 text-2xl font-bold text-sky-600">
                      ¥{Number(detail.price_new_jpy).toLocaleString()}
                    </div>
                  </div>
                )}
                {hasUsed && (
                  <div>
                    ...
                    <div className="mt-1 text-2xl font-bold text-amber-600">
                      ¥{Number(detail.price_used_jpy).toLocaleString()}
                      {typeof detail.price_used_max_jpy === "number" && (
                        <span className="text-base font-normal text-amber-500/80">
                          {" ~ "}¥{detail.price_used_max_jpy.toLocaleString()}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            {hasDual && (
              <p className="text-xs text-muted-foreground mb-2">{t("dualPriceNote")}</p>
            )}
          ...
          <Card className="p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">骏河屋 Surugaya</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <Button
              ...
              className={`gap-2 ${isCollected ? "text-amber-600 border-amber-200" : ""}`}
            >
