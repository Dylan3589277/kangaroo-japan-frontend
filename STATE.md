...
### 2026-09-05 · 客服页雅虎竞拍两处 P1 修复+对抗审查（ca63c5e）

**为什么**：对智能客服 H5（`/zh/support/auction/*`）雅虎竞拍/煤炉竞拍/雅虎即決做对抗
审查，结论主链路可用（4 天日志无真实客户出价/即決，均为测试流量），但发现两处前端
与 PHP 语义不一致的 P1：剩余秒数 0 被前端误当「已结束」（PHP 只把 `left_timestamp<0`
当结束，`0` 是「后端未知」）；「我的委托」翻页只等一个不存在的 `has_more` 字段，永远
只显示第一页。

**怎么做**：`src/app/[lang]/support/auction/[id]/page.tsx` 改为仅 `<0` 判结束，且倒计时
走完（本地计时到 0 后）也记为已结束；`mine/page.tsx` 翻页改按 PHP 返回的 `totalPages`
判断是否还有下一页。提交 `f0282ba`、`93426cc`，合并 `ca63c5e`。ECS 前端容器已拉新镜像
`832d83f9c67d`（回滚锚点 `fb93367f6e26`），页面 200，容器 bundle 已含「剩余时间未知」
字样。

**风险或遗留**：P2 留待未修——保证金不足按钮不禁用/错误显示原始数字、出价默认只加
10 円、redis 丢失报「错误的操作」、30 秒轮询详情、URL 签名 24h 无 nonce、
`/zh/mercari-auction` 不在 webview 白名单且需 JWT 而 bridge 引导客户去看。教训：改
`<=0` 为 `<0` 时没查上游的钳 0 逻辑，第一版漏了 `Math.max(0,…)`；对抗审查分身用了落后
本地仓（bridge `eec00ed`）误报两个 P0，需用 `origin/main` 与部署机复核才发现不成立。
...
（faq / fees / terms 页 / `docs/legal` 终稿）写的是「每天 5 元人民币」——其中 terms 页
那条是 `<RiskClause>` 标红的重大提示条款，**对外公示的收费标准与实际收费不一致**，
按民法典 496 条属于提示义务出问题的那一类，不是普通文案错字。

**怎么做**：前端 4 处统一改 100 日元/天（4 文件 4 行，无逻辑改动）。老后台 `KEFU_RULE`
同步三改：①汇率句补夜间优惠——常规 +0.0025，每日 19:00 至次日 9:00（中国时间）在小程序
自助下单煤炉商品享 +0.0023 ②超期仓储 5 元→100 日元 ③免费储存天数由「非会员 30 天/会员
60 天」改为「普通与黄金 30 天，白金与钻石 60 天」（原写法把黄金会员误归入 60 天档）。
改前备份 `runtime/KEFU_RULE.bak-nightrate-*.txt`，改后清 Redis `st_config` 键。

**验证**：库内真值逐句回读确认；Redis 已重建为新值（TTL 86279）；前端全仓
`grep "5 元|5元" | grep 仓储` 零命中；本地 `npm run build` exit 0。
（中途一次虚惊：在 JSON 转义串上做子串匹配，「日」的 `\u65e5` 末位 5 与「5元仓储」
的 5 相撞产生假阳性——**转义后的文本不能按原文子串匹配**。）

**遗留**：🔴 钻石会员 `rate` 仍为 0.0025（`update_time` 停在 2025-06，前三档 2026-08-04
已改 0.0023），花哥确认「漏改了，应该也享优惠」——会员定价数据由花哥在后台改，中枢不动。
