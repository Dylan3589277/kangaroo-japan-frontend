# 袋鼠君日本独立站 - 状态文档

> 版本: v1.0  
> 更新: 2026-04-16  
> 负责人: 花小弟 & Claude Code

---

## 项目概述

面向中日英三语的跨境电商比价独立站，聚合 Amazon/Mercari/Yahoo 三大日本电商平台的商品，支持多语言、多币种支付。

---

## 当前阶段

| 阶段 | 状态 | 开始时间 | 里程碑 |
|------|------|---------|--------|
| Phase 1 - 初始化 & 设计 | ✅ 完成 | 2026-04-16 | 完成项目初始化、技术选型、数据库设计、API设计、开发环境搭建 |
| Phase 2 - 核心功能开发 | 🔄 进行中 | 2026-04-16 | 完成用户认证、商品模块、购物车、订单、支付 |
| Phase 3 - 前后端联调 | ⏳ 待开始 | - | 完成API对接和页面开发 |
| Phase 4 - 测试 & 上线 | ⏳ 待开始 | - | 完成测试并部署 |

---

## 技术栈

| 层级 | 技术 | 备注 |
|------|------|------|
| 前端 | Next.js 14 (App Router) | TypeScript + Tailwind + shadcn/ui |
| 后端 | NestJS | TypeORM + PostgreSQL |
| 缓存 | Redis | Session + 缓存 |
| 搜索 | Algolia | 商品搜索 |
| 支付 | Stripe + Ping++ | 美元 + 人民币 |
| 部署 | Vercel + AWS ECS | 前端 + 后端 |

---

## Phase 2 任务分解

### 2.1 用户认证模块 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.1.1 用户注册/登录 API | ⏳ 待开始 | P0 |
| 2.1.2 JWT Token 管理 | ⏳ 待开始 | P0 |
| 2.1.3 第三方登录 (Google/Apple) | ⏳ 待开始 | P1 |
| 2.1.4 用户地址管理 (多国地址) | ⏳ 待开始 | P0 |

### 2.2 商品搜索与比价模块 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.2.1 商品数据模型 | ⏳ 待开始 | P0 |
| 2.2.2 多平台商品爬取/同步 | ⏳ 待开始 | P0 |
| 2.2.3 Algolia 搜索集成 | ⏳ 待开始 | P0 |
| 2.2.4 商品比价功能 | ⏳ 待开始 | P0 |
| 2.2.5 价格历史追踪 | ⏳ 待开始 | P1 |

### 2.3 购物车模块 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.3.1 购物车 CRUD | ⏳ 待开始 | P0 |
| 2.3.2 多商品结算 | ⏳ 待开始 | P0 |
| 2.3.3 按卖家分组 | ⏳ 待开始 | P0 |

### 2.4 订单模块 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.4.1 订单创建 | ⏳ 待开始 | P0 |
| 2.4.2 订单状态流转 | ⏳ 待开始 | P0 |
| 2.4.3 订单历史查询 | ⏳ 待开始 | P0 |

### 2.5 支付模块 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.5.1 Stripe 集成 (美元) | ⏳ 待开始 | P0 |
| 2.5.2 Ping++ 集成 (人民币) | ⏳ 待开始 | P0 |
| 2.5.3 支付回调处理 | ⏳ 待开始 | P0 |

### 2.6 前端页面开发 ✅ 规划中

| 子任务 | 状态 | 优先级 |
|--------|------|--------|
| 2.6.1 国际化布局 | ⏳ 待开始 | P0 |
| 2.6.2 首页开发 | ⏳ 待开始 | P0 |
| 2.6.3 商品列表/详情页 | ⏳ 待开始 | P0 |
| 2.6.4 购物车页面 | ⏳ 待开始 | P0 |
| 2.6.5 结算流程 | ⏳ 待开始 | P0 |

---

## 已完成的工作

### Phase 1 完成项

- [x] 需求分析 + 代码审计
- [x] 技术选型确认
- [x] 数据库设计 (PostgreSQL)
- [x] API 设计 (RESTful)
- [x] 开发环境搭建
  - [x] Next.js 14 前端项目
  - [x] NestJS 后端项目
  - [x] Docker Compose 配置
  - [x] GitHub Actions CI/CD
  - [x] Husky + lint-staged

---

## 下次行动

### Phase 2 启动任务

- [ ] 2.1.1 用户注册/登录 API - Claude Code
- [ ] 2.1.2 JWT Token 管理 - Claude Code
- [ ] 2.1.4 用户地址管理 (多国地址) - Claude Code

---

## 关键资源

| 资源 | 位置 |
|------|------|
| 前端源码 | /Users/hulonghua/workspace/kangaroo-japan/ |
| 后端源码 | /Users/hulonghua/workspace/kangaroo-japan-backend/ |
| 设计文档 | /Users/hulonghua/.openclaw/workspace/shared/袋鼠君独立站项目/ |
| 项目索引 | registry.yaml (待创建) |

---

> ⚠️ **事实层**：此文档为唯一真相源，所有展示层（飞书等）由此同步生成。
